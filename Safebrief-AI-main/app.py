...
if uploaded_file and st.session_state.generated:
    ...
    with col2:
        ...
        st.info("""
        • Real-time industrial hazard analysis completed

        • Ensemble machine learning prediction active

        • Safety compliance monitoring enabled

        • Predictive operational intelligence running

        • Executive-level incident assessment generated
        """)
    ...
    # HISTORICAL ANALYSIS
    st.markdown(
        "<div class='section-title'>Similar Past Incident Analysis</div>",
        unsafe_allow_html=True
    )
    ...
    if similar_incident:
        ...
        st.info(f"""
        Similar Past Incident Detected:

        {similar_incident['incident']}
        """)

        st.success(f"""
        Recommended Historical Handling Strategy:

        {similar_incident['solution']}
        """)

    else:

        st.warning("No similar historical incident found.")

        st.markdown(
            "<div class='section-title'>AI Safety Recommendations</div>",
            unsafe_allow_html=True
        )

        st.success(ai_solution)
